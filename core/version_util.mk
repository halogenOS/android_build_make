...
ifdef RELEASE_PLATFORM_SECURITY_PATCH_OVERRIDE
  ifeq ($(shell [[ $(RELEASE_PLATFORM_SECURITY_PATCH) > $(RELEASE_PLATFORM_SECURITY_PATCH_OVERRIDE) ]] && echo -n 1),1)
    # Original patch is newer than override, set to latest
    $(warning Security patch override $(RELEASE_PLATFORM_SECURITY_PATCH_OVERRIDE) is older than original patch $(RELEASE_PLATFORM_SECURITY_PATCH), undoing override. Please fix in device tree.)
    PLATFORM_SECURITY_PATCH := $(RELEASE_PLATFORM_SECURITY_PATCH)
  else
    $(shell echo Overriding security patch $(RELEASE_PLATFORM_SECURITY_PATCH) with $(RELEASE_PLATFORM_SECURITY_PATCH_OVERRIDE) >&2)
    PLATFORM_SECURITY_PATCH := $(RELEASE_PLATFORM_SECURITY_PATCH_OVERRIDE)
  endif
else
PLATFORM_SECURITY_PATCH := $(RELEASE_PLATFORM_SECURITY_PATCH)
endif
